...
class TestAddress(TransactionTestCase):
    ...
    def test_post_new_data(self, mock_logger):
        ...
        self.assertEqual(len(ShippingAddress.objects.filter(is_default=True, user=self.user)), 1)
    ...
    def test_put(self, mock_logger):
        self.handle_login()
        res_put = self.client.put(reverse("shipping_address", args=[1]), data={
            "province_name": "NUSA TENGGARA BARAT (NTB)",
            "city_name": "MATARAM",
            "district_name": "MATARAM",
            "subdistrict_name": "MATARAM",
    ...
    def test_put_is_default(self, mock_logger):
        ...
        """
        self.handle_login(self.user2)
        res_put = self.client.put(reverse("shipping_address", args=[3]), data={
            "province_name": "NUSA TENGGARA BARAT (NTB)",
            "city_name": "MATARAM",
            "district_name": "MATARAM",
            "subdistrict_name": "MATARAM",
        ...
        # mengecek data yang is_default true hanya ada 1
        self.assertEqual(len(ShippingAddress.objects.filter(is_default=True, user=self.user2)), 1)
