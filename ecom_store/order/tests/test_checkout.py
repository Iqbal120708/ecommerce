...
class CheckoutTest(TransactionTestCase):
    ...
    def test(self, order_logger_error_util, order_log_util,order_logger_error_view, order_log_view, mock_logger):
        self.handle_login()
        # add cart
        ...
        # update cart
        res_patch = self.client.patch(reverse("cart", args=[res_add.data["id"]]), data={"qty": 3})
        self.assertEqual(res_patch.status_code, 200)

        # checkout
        res = self.client.post(reverse("checkout"), data={"cart_ids": [res_add.data["id"]]}, format='json')
        ...
        self.assertEqual(logs[1].args[0], f"Checkout ID {res.data['checkout_id']} dibuat untuk User 1. Data disimpan di cache.")
    ...
    @patch("order.utils.logger")
    @patch("order.utils.logger_error")
    def test_with_ship_addr_id(self, order_logger_error_util, order_log_util,order_logger_error_view, order_log_view, mock_logger):
        """
        ...
        # checkout
        res = self.client.post(reverse("checkout"), data={"cart_ids": [res_add.data["id"]], "shipping_address_id": self.shipping_address.id}, format='json')
    ...
    @patch("order.utils.logger")
    @patch("order.utils.logger_error")
    def test_error_data_cart_ids(self, order_logger_error_util, order_log_util,order_logger_error_view, order_log_view, mock_logger):
        self.handle_login()
        # add cart
        res_add = self.client.post(reverse("add_to_cart", args=[1]), data={})
        self.assertEqual(res_add.status_code, 201)
        ...
        self.assertEqual(res.data["error"], "cart_ids harus berupa list dan tidak boleh kosong.")
        ...
        res = self.client.post(reverse("checkout"), data={"cart_ids": ["a"]}, format='json')
        self.assertEqual(res.status_code, 400)
        self.assertEqual(res.data["error"], "Semua item di dalam cart_ids harus berupa angka (integer).")
        ...
        self.assertEqual(logs[0].args[0], "User 1 memulai checkout untuk cart_ids: None")
        self.assertEqual(logs[1].args[0], "User 1 memulai checkout untuk cart_ids: 1")
        self.assertEqual(logs[2].args[0], "User 1 memulai checkout untuk cart_ids: ['a']")
    ...
    @patch("order.utils.logger")
    @patch("order.utils.logger_error")
    def test_error_store_origin(self, order_logger_error_util, order_log_util,order_logger_error_view, order_log_view, mock_logger):
        """
        ...
        order_logger_error_util.error.assert_called()
        args, kwargs = order_logger_error_util.error.call_args
    ...
    @patch("order.utils.logger")
    @patch("order.utils.logger_error")
    def test_error_destination(self, order_logger_error_util, order_log_util,order_logger_error_view, order_log_view, mock_logger):
        """
        ...
        self.assertEqual(res.status_code, 400)
        self.assertEqual(res.data["error"], "Alamat pengiriman belum dipilih. Silakan pilih salah satu alamat Anda atau atur salah satu sebagai 'Alamat Utama' (Default).")
        ...
        self.assertEqual(args[0], "Destination tidak ditemukan. User ID: 1, Address ID Provided: None")
    ...
    def test_error_api_key(self, order_logger_error_util, order_log_util,order_logger_error_view, order_log_view, mock_logger):
        ...
        self.assertEqual(args[0], "Gagal mengambil ongkir RajaOngkir untuk User 1. Error: Invalid Api key, key not found")
