...
from .utils import (create_order, create_order_details, create_order_item,
                    create_order_rajaongkir,
...
class CheckoutView(APIView):
    ...
    def post(self, request):
        ...
        logger.info(
            f"Checkout ID {checkout_id} dibuat untuk User {request.user.id}. Data disimpan di cache."
        )
        return Response(
            {"checkout_id": checkout_id, "shipping_options": shipping_options}
        )
...
class TransactionView(APIView):
    def post(self, request):
        ...
        order = create_order(
            request.user, shipping_context, serializer.data, store, order_details
        )
